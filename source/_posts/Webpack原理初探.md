...
​		本文转载自`SegmentFault`的[**whjin**](https://segmentfault.com/u/whjin)，原文地址：https://segmentfault.com/a/1190000015088834#articleHeader17。

<!--more-->

### 是什么

初学 `webpack` 的小伙伴想必和我有一样的问题：`webpack` 是什么？

​		简而言之，`webpack` 算是一个集成打包、压缩功能的前端编译工具。而真正将其 引入历史舞台的原因，引用表严肃表哥的一句话：`浏览器实现之初没有做到模块化，之后都是在补作业` 。所以 `webpack` 的作用我们就可以理解为：使前端页面得以通过模块化加载方式进行载入。

​	当然，打包压缩什么的算是后话，原理暂时这么记。

### 几个概念

- `Entry`： 入口文件
- `Module`：最终打包完成的`bundle`放至何处
- `Chunk`：代码块。一个`Chunk`由多个模块组合而成，用于代码合并与分割。
- `Loader`：用`loader`处理非 JS 文件，将其转化为`webpack`所能处理的有效模块（JS、JSX）
- `Plugin`：插件。用于打包优化、压缩、重新定义环境中的变量等等。。。
...
2. 开始编译：用上一步得到的参数初始化`Complier`对象，加载所有的配置的插件，执行对象的`run `方法开始执行编译
3. 确定入口：找`entry`
4. 编译模块：从入口文件出发，调用所有配置的`loader`对模块进行编译，再找出该模块的依赖模块，直至所有入口文件的依赖文件都经过本步骤的处理
5. 完成模块编译：经过`Loader`翻译完所有模块后得到的是他们被翻译后的内容及它们之间的依赖关系
6. 输出资源：根据入口和模块之间的依赖关系组成一个个包含多个模块的`Chunk`，再把美个`Chunk`转换成一个单独的文件加入到输出列表。这步是修改输出内容的最后机会
...
3. 输出：将编译后的`Module`组合成`Chunk`，把`Chunk`转换成文件，输出到文件系统。

当然，如果你开启了监听模式，那么整个流程会变成这样

![](https://ae01.alicdn.com/kf/Ha80f13a83fa7456089be842c12a8af55u.png)
